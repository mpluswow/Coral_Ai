...
import json
from modules.user_logs import log_interaction
...
def initialize_chatbot():
    ...
    coral_data = train_chatbot(trainer, 'training_data/coral_reefs_data.json')

    # Load existing training data for the team from JSON file
    team_data = train_chatbot(trainer, 'training_data/team_data.json')
...
def get_best_match(user_input, coral_data, team_data):
    ...
    try:
        ...
        # Print the exception type and message for better debugging
        print(f"Error in fuzzy matching: {type(e).__name__} - {e}")
